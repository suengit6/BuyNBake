document.addEventListener('DOMContentLoaded', async () => {
  const stripePublicKey = 'pk_test_51Nfo6zGgyMo6QIJGM3nvDLBvwN2ASnKILTbBzIEeCqGA87Ppt6Ki6SqaFQFLG2LoeZNMfspCl6QpDbSPi02cBtae00Fe249aON';
  const stripe = Stripe(stripePublicKey);
  const checkoutButton = document.getElementById('checkout-button');

  checkoutButton.addEventListener('click', async () => {
    // Retrieve user ID and address from localStorage
    const userAccount = JSON.parse(localStorage.getItem('userAccount')) || {};
    const userId = userAccount.userId;
    const userAddress = userAccount.address || 'N/A';

    // Retrieve product information from localStorage using the user's ID
    ...
    for (const productName in cart) {
      totalProductPrice += cart[productName].price * cart[productName].quantity;
    }
